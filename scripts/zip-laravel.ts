...
async function zipFolder() {
  console.log('Starting JSZip packaging for laravel-backend...');
  const zip = new JSZip();
  const folderPath = path.resolve('laravel-backend');
  ...
  fs.writeFileSync('public/laravel-backend.zip', content);
  console.log('Zip file successfully generated inside public/laravel-backend.zip');
}
